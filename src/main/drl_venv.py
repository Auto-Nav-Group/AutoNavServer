...
class RobotVEnv:
    ...
    def step(self, action):
        ...
        robot_state = [theta, self.x, self.y, self.goal_x, self.goal_y, self._run_lidar(), action[0], action[1]]
        # reward = self.get_reward(target, collision, action)
        # return robot_state, reward, done, target
        return robot_state, collision, done, achieved_goal, dist_traveled
    ...
    def reset(self): # Create a new environment
        ...
        angle = self._reset_situation(np.pi)
        ...
        robot_state = [theta, self.x, self.y, self.goal_x, self.goal_y, self._run_lidar(), 0, 0]
        return robot_state, distance
    ...
    def _reset_situation(self, ideal_angle, new_goal=True):
        x = 0
        y = 0
        position_fine = False
        while not position_fine:
